...
//    enum
    ...
    private func updateView(_ view: CurrentWeatherView) {
        ...
        self.getImage(iconNumber: Int(data.weatherIcon)) { weatherIcon in
    ...
    private func updateView(_ views: [OneHourView]) {
        ...
        for (index, view) in views.enumerated() {
            ...
            self.getImage(iconNumber: Int(data[index].weatherIcon)) { weatherIcon in
                view.setIcon(weatherIcon)
            }
        }
    }
    ...
    private func updateView(_ views: [OneDayView]) {
        ...
        for (index, view) in views.enumerated() {
            ...
            self.getImage(iconNumber: Int(data[index].dayWeather.weatherIcon)) { dayIcon in
                view.setDayIcon(dayIcon)
            }
            self.getImage(iconNumber: Int(data[index].nightWeather.weatherIcon)) { nightIcon in
    ...
    private func parseCurrentWearher(from dataArray: [[String : Any]]?) -> CurrentWeather? {
        ...
        let currentWeather = CurrentWeather(for: self.city,
                                            isDayTime: isDayTime,
    ...
    private func parseHourlyForecastArray(from dataArray: [[String : Any]]?) -> [HourlyForecast]? {
        ...
        for dataDictionary in dataArray {
            ...
            let hourlyForecastItem = HourlyForecast(for: self.city,
                                                       forecastTime: date,
                                                       temperatureCelsius: temperatureCelsius,
                                                       weatherIcon: weatherIcon,
                                                       weatherText: weatherText)
    ...
    private func parseDailyForecastDictionary(from data: [String : Any]?) -> [DailyForecast]? {
        ...
        for dataDictionary in dataArray {
            ...
            let dailyForecastItem = DailyForecast(for: self.city,
                                                  forecastDate: date,
